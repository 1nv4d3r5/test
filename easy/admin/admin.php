...
function generate() {
	...
	while($menu[$count][0]!="") {
		...
		if(!strval(strstr($menu[$count][3], "*"))) {
			...
			$page="<?php \$pagenum=\"".$pagenum."\"; require_once \"./admin/runtime.php\"; ?>\n";
...
function adminmenu() {
	...
	if($_SESSION[$set['password']]=="1") {
		...
		$aa.="<img src=\"images/tools.png\" alt=\"Settings\" align=\"left\" title=\""."Go to Admin Panel"."\" /></a></td>\n\t";
		$aa.="</tr>\n</table></div>\n";
	}
	return $aa;
}
...
function settings() {
	...
	$msg = "Admin Panel";
	$out.="<h2>$msg</h2><hr /><div id=\"LNE_admininput\">\n";
	$out.="<div class=\"LNE_settings\"><a href=\"".$set['homepath'].$set['indexfile']."?page=".$pagenum."&amp;do=create\"><img src=\"images/addpage.png\" alt=\"Create Page\" title=\"Create Page\" style=\"border: 0;\" /></a><br />"."Create Page"."</div>\n";
...
function users() {
	...
	if($pages>25) {
		...
		$pagebar="<tr><td colspan=\"6\" align=\"left\"><a href=\"main.php?page=index&do=users";
		if(isset($_GET['letter']))
		...
		while($row = fetch_array($result1)) {
			$pagebar.="<a href=\"main.php?page=index&do=users&letter=".$row['letter']."&pag=1\">".$row['letter']."</a> ";
		}
		$pagebar.=" <a href=\"main.php?page=index&do=users";
		if(isset($_GET['letter']))
			$pagebar.="&letter=".sanitize($_GET['letter']);
		$pagebar.="&pag=";
